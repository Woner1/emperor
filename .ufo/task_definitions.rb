...
#
task_definition "emperor-web" do
  ...
  variables(
    ...
    awslogs_group: ["ecs/emperor-web", Ufo.env_extra].compact.join('-'),
    awslogs_stream_prefix: "emperor",
    awslogs_region: helper.current_region,
    # command: ["bin/web"] # IMPORTANT: change or create a bin/web file
  )
end

task_definition "emperor-worker" do
  source "main" # will use ufo/templates/main.json.erb
  variables(
    family: task_definition_name,
    name: "worker",
    # Comment out awslogs_* if you do not want logs to be sent to CloudWatch.
    awslogs_group: ["ecs/emperor-worker", Ufo.env_extra].compact.join('-'),
    awslogs_stream_prefix: "emperor",
    awslogs_region: helper.current_region,
...
task_definition "emperor-clock" do
  source "main" # will use ufo/templates/main.json.erb
  variables(
    family: task_definition_name,
    name: "clock",
    # Comment out awslogs_* if you do not want logs to be sent to CloudWatch.
    awslogs_group: ["ecs/emperor-clock", Ufo.env_extra].compact.join('-'),
    awslogs_stream_prefix: "emperor",
    awslogs_region: helper.current_region,
